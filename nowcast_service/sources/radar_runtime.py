...
class RadarSourceRunner:
    ...
    async def run(self, *, evaluated_at: Any) -> SourceSnapshot:
        ...
        from nowcast_service.sources.dwd_directory import RV_SPEC, DwdDirectoryClient
                ...
                try:
                    downloaded = await download_atomic(
                        ...
                        limits=DownloadLimits(
                            max_bytes=64 * 1024 * 1024, allowed_suffixes=(".tar",)
                        ),
                    )
                    if downloaded is None:
                        raise SourceRunError(
                            "RADAR_NOT_MODIFIED_WITHOUT_CACHE", "No cached archive"
                        )
                    temp_root = self._data_dir / "temp"
                    temp_root.mkdir(parents=True, exist_ok=True)
                    with tempfile.TemporaryDirectory(dir=temp_root) as temporary:
                        members = await asyncio.to_thread(
                            extract_tar_safely, downloaded.path, Path(temporary)
                        )
                        frames = await asyncio.gather(
                            *(
                                asyncio.to_thread(
                                    load_radar_frame, member, expected_product="DWD_RV"
                                )
                    ...
                    payload: dict[str, Any] = {
                        ...
                        "rainNow": analysis.rain_now,
                        "movingTowardSite": analysis.moving_toward_site,
                    ...
                    return SourceSnapshot(
                        ...
                        valid_until=reference + timedelta(seconds=invalid),
