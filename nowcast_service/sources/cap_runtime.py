...
def _cap_evidence(alerts: tuple[Any, ...]) -> tuple[Evidence, ...]:
    ...
    for alert in alerts:
        ...
        risk = RiskState.RED if critical or info.severity in {"Severe", "Extreme"} else RiskState.YELLOW
        evidence.append(Evidence(source="DWD_CAP", state=risk,
            reason_code="CAP_RELEVANT_WARNING_RED" if risk is RiskState.RED else "CAP_RELEVANT_WARNING_YELLOW",
...
class CapSourceRunner:
    ...
    async def run(self, *, evaluated_at: Any) -> SourceSnapshot:
        ...
        from nowcast_service.sources.dwd_cap_directory import CAP_COMMUNE_SPEC, DwdCapDirectoryClient
        from nowcast_service.sources.warning_area_index import DwdWarningAreaClient

        timeout = httpx.Timeout(30.0, connect=10.0)
        errors: list[str] = []
        async with httpx.AsyncClient(timeout=timeout, trust_env=False) as client:
            if self._warning_index is None or self._warning_index_loaded_at is None or (evaluated_at - self._warning_index_loaded_at) > timedelta(hours=24):
                try:
            ...
            for candidate in candidates[:5]:
                try:
                    downloaded = await download_atomic(client, url=candidate.url,
                        ...
                        limits=DownloadLimits(max_bytes=32 * 1024 * 1024, allowed_suffixes=(".zip",)))
                    if downloaded is None:
                        raise SourceRunError("CAP_NOT_MODIFIED_WITHOUT_CACHE", "No cached archive")
                    result = await asyncio.to_thread(load_cap_archive, downloaded.path,
                        now=evaluated_at, longitude=self._config.location.longitude,
                        latitude=self._config.location.latitude, resolver=self._warning_index)
                    unresolved = tuple(alert for alert in result.location.unresolved
                        if alert.german_info() is not None and alert.german_info().is_in_force(evaluated_at))
                    if unresolved:
                        raise SourceRunError("CAP_LOCATION_UNRESOLVED", "At least one in-force CAP area could not be resolved")
                    ...
                    for alert in result.location.matched:
                        ...
                        warnings.append({"identifier": alert.identifier, "event": info.event,
                            "severity": info.severity, "urgency": info.urgency,
                            "certainty": info.certainty, "effective": iso(info.effective),
                            "onset": iso(info.onset), "expires": iso(info.expires),
                            "headline": info.headline, "description": info.description,
                            "instruction": info.instruction, "source": "Deutscher Wetterdienst"})
                    payload: dict[str, Any] = {"active": warnings, "activeCount": len(warnings),
                        "archiveEntries": result.archive_entries, "parsedAlerts": result.parsed_alerts,
                    ...
                    return SourceSnapshot(source_id=self.source_id,
                        ...
                        product="DWD_CAP_COMMUNE", cycle_time=cycle_time,
                        downloaded_at=evaluated_at, parsed_at=evaluated_at,
                        valid_from=evaluated_at, valid_until=evaluated_at + timedelta(seconds=invalid),
                        evaluated_at=evaluated_at, state=SourceState.LIVE,
                        content_sha256=downloaded.sha256, is_complete=True,
                        stale_after_seconds=stale, invalid_after_seconds=invalid,
                        payload=payload, evidence=_cap_evidence(tuple(active_matched)))
                except Exception as exc:
                    errors.append(f"{candidate.name}:{type(exc).__name__}")
        raise SourceRunError("CAP_CYCLE_FAILED",
