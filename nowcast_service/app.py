...
def _production_runners(config: AppConfig, data_dir: Path) -> tuple[SourceRunner, ...]:
    ...
    return (RadarSourceRunner(config=config, data_dir=data_dir),
            CapSourceRunner(config=config, data_dir=data_dir))


def create_app(*, data_dir: Path | None = None, enable_background: bool = False,
               source_runners: tuple[SourceRunner, ...] | None = None) -> FastAPI:
    ...
    coordinator = RuntimeCoordinator(config=config, data_dir=config_store.data_dir,
    ...
    application = FastAPI(title="Astro-Wolkencheck local service", version=__version__,
    ...
    def meta() -> dict[str, object]:
        return {"schemaVersion": SCHEMA_VERSION, "appVersion": __version__,
                "mode": "LOCAL", "generatedAt": utc_now()}
    ...
    def health_ready() -> dict[str, object]:
        ...
        return {"status": "READY",
            ...
            "snapshotId": snapshot.snapshot_id, "generatedAt": utc_now()}
    ...
    def get_session() -> dict[str, object]:
        snapshot = coordinator.snapshot()
        return {"schemaVersion": SCHEMA_VERSION, "snapshotId": snapshot.snapshot_id,
                "equipmentState": coordinator.equipment_state, "generatedAt": utc_now()}
    ...
    async def patch_session(patch: SessionPatch) -> dict[str, object]:
        ...
        snapshot = await coordinator.set_equipment_state(patch.equipment_state,
            configuration_version=updated.configuration_version)
        return {"schemaVersion": SCHEMA_VERSION, "snapshotId": snapshot.snapshot_id,
                "equipmentState": coordinator.equipment_state, "generatedAt": utc_now()}

    @application.get("/api/v1/safety")
    def safety() -> dict[str, object]: return coordinator.snapshot_dict()
    @application.get("/api/v1/runtime")
    def runtime() -> dict[str, object]: return coordinator.runtime_dict()
    @application.get("/api/v1/nowcast")
    def nowcast() -> dict[str, object]: return coordinator.snapshot_dict()
    @application.get("/api/v1/radar")
    def radar() -> dict[str, object]: return coordinator.radar_dict()
    @application.get("/api/v1/warnings")
    def warnings() -> dict[str, object]: return coordinator.warnings_dict()
    @application.get("/api/v1/sources")
    def sources() -> list[dict[str, object]]: return coordinator.source_dicts()
    @application.get("/api/v1/alerts")
    def alerts() -> dict[str, object]: return coordinator.alerts_dict()
    ...
    def diagnostics() -> dict[str, object]: return coordinator.diagnostics_dict()

    @application.get("/api/v1/config/public")
    def public_config() -> dict[str, object]:
        current = config_store.load()
        return {"schemaVersion": current.schema_version,
            "location": {"name": current.location.name, "timezone": current.location.timezone,
    ...
    def events(last_event_id: str | None = Header(default=None)) -> StreamingResponse:
        ...
        async def generate() -> AsyncIterator[str]:
            initial = json.dumps({"eventId": 0, "eventType": "snapshot",
                "createdAt": utc_now(), "payload": coordinator.snapshot_dict()}, separators=(",", ":"))
            if cursor == 0:
                yield f"id: 0\nevent: snapshot\ndata: {initial}\n\n"
    ...
    def root() -> HTMLResponse:
        ...
        if "</head>" in content: content = content.replace("</head>", style + "</head>", 1)
        if "</body>" in content: content = content.replace("</body>", script + "</body>", 1)
        else: content += script
