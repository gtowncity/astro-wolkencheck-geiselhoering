(() => {
  ...
  function renderWarnings(snapshot, api) {
    ...
    if (count) count.textContent = `${warnings.length} aktiv · ${hardwareCount} Hardware-Veto${hardwareCount === 1 ? "" : "s"}`;
    ...
    articles.forEach((article, index) => {
      ...
      const rows = [
        ...
        ["Dringlichkeit", URGENCY[warning.urgency] || warning.urgency || "nicht angegeben"],
        ["Sicherheit", CERTAINTY[warning.certainty] || warning.certainty || "nicht angegeben"],
        ["Gültig ab", time(warning.effective || warning.sent)],
        ["Erwarteter Beginn", time(warning.onset)],
        ["Ende", time(warning.expires, "keine feste Ablaufzeit im aktuellen vollständigen DWD-Archiv")],
        ["Quelle", warning.senderName || warning.sender || "Deutscher Wetterdienst (DWD)"],
      ];
  ...
  function renderHazards(snapshot) {
    ...
    list.replaceChildren(...hazards.map((hazard) => {
      const item = document.createElement("li");
      item.className = "awc-hazard-entry";
      const heading = document.createElement("strong");
      heading.textContent = hazard.reason || hazard.reasonCode || "Gefahr bleibt vorsorglich aktiv";
      ...
      facts.append(
        ...
        fact("Quittiert", hazard.acknowledgedAt ? time(hazard.acknowledgedAt) : "nein"),
      );
      const note = document.createElement("small");
      note.textContent = hazard.clearCondition || "Die Gefahr endet erst nach den festgelegten frischen Entwarnungszyklen.";
  ...
  function ensureNotificationTest(buttonRow) {
    ...
    button.addEventListener("click", async () => {
      ...
      if (typeof Notification === "undefined") {
        if (status) status.textContent = "Browsermeldungen werden von diesem Browser nicht unterstützt.";
        return;
      }
      let permission = Notification.permission;
      if (permission === "default") permission = await Notification.requestPermission();
      if (permission !== "granted") {
        if (status) status.textContent = "Browsermeldung ist nicht erlaubt. Bitte Browserberechtigung prüfen.";
        return;
      }
      ...
      if (status) status.textContent = "Test-Browsermeldung wurde ausgelöst. Es wurde keine Gefahr erzeugt.";
    });
  ...
  function renderAlarm(snapshot) {
    ...
    const browserPermission = typeof Notification === "undefined" ? "nicht verfügbar" : Notification.permission;
    const api = window[DASHBOARD];
    const enabled = Boolean(api?.getState?.().alarmsEnabled);
    capabilities.replaceChildren(
      capability("Akustischer Alarm", enabled ? "aktiv" : "nicht aktiviert", enabled ? "GREEN" : "YELLOW"),
      capability(
        "Browsermeldung",
        browserPermission === "granted" ? "erlaubt" : browserPermission === "denied" ? "blockiert" : browserPermission,
        browserPermission === "granted" ? "GREEN" : browserPermission === "denied" ? "RED" : "YELLOW",
      ),
      capability("Aktiver Gefahren-Latch", snapshot?.activeHazards?.length ? "ja" : "nein", snapshot?.activeHazards?.length ? "RED" : "GREEN"),
    );
    const enable = document.getElementById("awc-enable-alerts");
    const test = document.getElementById("awc-test-alarm");
    if (enable) enable.textContent = enabled ? "Alarmierung aktiviert" : "Alarmierung einrichten";
    ...
    if (!note) {
      ...
      note.textContent = "Quittieren beendet Ton und Browseralarm, aber niemals die erkannte Gefahr oder deren Haltezeit.";
  ...
  function render() {
    ...
    const signature = JSON.stringify({
      ...
      warnings: warnings.map((warning) => [warning.identifier, warning.urgency, warning.certainty]),
      hazards: (snapshot.activeHazards || []).map((hazard) => [hazard.hazardKey, hazard.clearStreak, hazard.acknowledgedAt]),
