...
def metadata(product: str) -> dict[str, object]:
    available = [
        {
            "key": "geocolour",
    ...
    selected = next(item for item in available if item["key"] == product)
    return {
        "provider": "EUMETSAT",
...
<body>
  ...
  <script src='/local-ui-recovery.js' defer></script>
  <script src='/local-satellite-viewer.js' defer></script>
</body>
</html>"""
...
def install_fullscreen_stub(page: Page) -> None:
    page.add_init_script(
        ...
        Object.defineProperty(document, 'fullscreenElement', {
          get() { return window.__fullscreenElement; },
          configurable: true,
        });
...
def test_deferred_forecast_compact_layout_and_satellite_controls() -> None:
    ...
    def route_request(route: Route) -> None:
        ...
        if path == "/":
            ...
            route.fulfill(
                ...
                body=(STATIC / path.removeprefix("/")).read_text(encoding="utf-8"),
            )
        elif path in {
            ...
            route.fulfill(
                ...
                body=json.dumps(metadata(product)),
            )
    ...
    with sync_playwright() as playwright:
        ...
        forecast_requests = [url for url in requests if "open-meteo.com" in url]
        assert forecast_requests == []
        ...
        hazard_display = page.locator(".awc-hazard-entry").evaluate(
        ...
        page.wait_for_function(
            "document.getElementById('awc-satellite-product').value === 'infrared'"
        )
