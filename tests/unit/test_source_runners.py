...
class DummyClient:
    def __init__(self, *args: object, **kwargs: object) -> None:
        del args, kwargs
    async def __aenter__(self) -> "DummyClient": return self
    async def __aexit__(self, *args: object) -> None: del args
...
def install_radar_modules(monkeypatch: pytest.MonkeyPatch, *, reference: datetime,
    incomplete: bool = False, directory_error: bool = False, no_download: bool = False) -> None:
    candidate = SimpleNamespace(name="composite_rv_LATEST.tar", url="https://example", reference_time=None)
    class DirectoryClient:
        def __init__(self, client: object) -> None: del client
        async def candidates(self, spec: object) -> tuple[object, ...]:
            del spec
            if directory_error: raise RuntimeError("directory")
            return (candidate,)
    async def download_atomic(*args: object, **kwargs: object) -> object | None:
        del args, kwargs
        if no_download: return None
    ...
    arrival = SimpleNamespace(earliest_minutes=35, estimate_minutes=40,
        latest_minutes=45, confidence="MEDIUM")
    current = SimpleNamespace(lead_minutes=0,
        nearest_component=SimpleNamespace(nearest_distance_km=12.5,
            nearest_bearing_deg=91.0, area_km2=8.0), site_maximum_mm_5min=0.0)
    analysis = SimpleNamespace(missing_leads=(5,) if incomplete else (),
        coverage_complete_to_60=not incomplete, coverage_complete_to_120=not incomplete,
        available_leads=(0, 5, 120), frames=(current,), arrival=arrival,
        rain_now=False, moving_toward_site=True, maximum_site_amount_mm_5min=0.2,
    ...
    monkeypatch.setitem(sys.modules, "nowcast_service.downloads.remote",
        module("nowcast_service.downloads.remote", DownloadLimits=lambda **kwargs: kwargs,
            download_atomic=download_atomic))
    monkeypatch.setitem(sys.modules, "nowcast_service.downloads.safe_tar",
        module("nowcast_service.downloads.safe_tar", extract_tar_safely=extract_tar_safely))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.dwd_directory",
        module("nowcast_service.sources.dwd_directory", DwdDirectoryClient=DirectoryClient, RV_SPEC=object()))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.radar_hdf5",
        module("nowcast_service.sources.radar_hdf5", load_radar_frame=load_radar_frame))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.radar_analysis",
        module("nowcast_service.sources.radar_analysis", analyze_radar_cycle=analyze_radar_cycle,
...
async def test_radar_runner_maps_freshness(tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    age_minutes: int, expected: SourceState) -> None:
    ...
    snapshot = await RadarSourceRunner(config=AppConfig(), data_dir=tmp_path).run(evaluated_at=evaluated)
...
@pytest.mark.asyncio
async def test_radar_runner_rejects_incomplete_and_discovery_failures(tmp_path: Path,
...
class CapInfo:
    def __init__(self, *, event: str = "GEWITTER", in_force: bool = True, expires: datetime) -> None:
        self.event, self.severity, self.urgency, self.certainty = event, "Minor", "Immediate", "Likely"
        self.effective = self.onset = expires - timedelta(hours=1)
        self.expires, self.headline, self.description, self.instruction = expires, event, "Official text", "Protect equipment"
...
class CapAlert:
    ...
    def german_info(self) -> CapInfo: return self._info


def install_cap_modules(monkeypatch: pytest.MonkeyPatch, *, evaluated: datetime,
    matched: tuple[CapAlert, ...] = (), unresolved: tuple[CapAlert, ...] = (),
    warning_area_error: bool = False, directory_error: bool = False,
    ...
    class WarningClient:
        ...
        async def fetch(self, **kwargs: object) -> object:
            del kwargs
            if warning_area_error: raise RuntimeError("wfs")
            return object()
    class DirectoryClient:
        def __init__(self, client: object) -> None: del client
        async def candidates(self, spec: object) -> tuple[object, ...]:
    ...
    result = SimpleNamespace(location=SimpleNamespace(matched=matched, unresolved=unresolved),
        archive_entries=3, parsed_alerts=3)
    ...
    monkeypatch.setitem(sys.modules, "nowcast_service.downloads.remote",
        module("nowcast_service.downloads.remote", DownloadLimits=lambda **kwargs: kwargs,
            download_atomic=download_atomic))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.cap_snapshot",
        module("nowcast_service.sources.cap_snapshot", load_cap_archive=load_cap_archive))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.dwd_cap_directory",
        module("nowcast_service.sources.dwd_cap_directory", DwdCapDirectoryClient=DirectoryClient,
            CAP_COMMUNE_SPEC=object()))
    monkeypatch.setitem(sys.modules, "nowcast_service.sources.warning_area_index",
        module("nowcast_service.sources.warning_area_index", DwdWarningAreaClient=WarningClient))


@pytest.mark.asyncio
async def test_cap_runner_filters_future_and_maps_active_warning(tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_cap_runner_rejects_unresolved_and_discovery_failures(tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch) -> None:
    now = datetime(2026, 8, 5, 8, 0, tzinfo=UTC)
    ...
    for warning, directory, expected in ((True, False, "CAP_WARNING_AREA_FAILED"),
                                         (False, True, "CAP_DIRECTORY_FAILED")):
        install_cap_modules(monkeypatch, evaluated=now, warning_area_error=warning,
