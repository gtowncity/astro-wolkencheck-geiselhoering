...
import pytest
from nowcast_service.decision_engine import Action, DataQuality, EquipmentState, RiskState, SafetyDecision, SourceState
...
def source(now: datetime) -> SourceSnapshot:
    return SourceSnapshot("DWD_RV", "rv:1", "DWD_RV", now, now, now, now,
        now + timedelta(minutes=30), now, SourceState.LIVE, "a" * 64, True, 900, 1800)
...
def test_database_round_trip_health_and_invalid_payload(tmp_path: Path) -> None:
    ...
    signal = HazardSignal("rv:red", "DWD_RV", RiskState.RED, "R", "Rain", now,
        now + timedelta(minutes=30), source_input_id="rv:1")
    hazard = LatchedHazard(signal, now, now)
    snapshot = DecisionSnapshot.create(evaluation_at=now, algorithm_version="test",
        configuration_version="1", equipment_state=EquipmentState.NOT_DEPLOYED,
        decision=SafetyDecision(RiskState.RED, DataQuality.COMPLETE, Action.DO_NOT_SETUP,
            ("R",), ("Rain",)), active_hazards=(hazard,), source_snapshots=(source(now),))
    ...
    with sqlite3.connect(db.path) as connection:
        ...
        connection.execute("INSERT INTO hazard_latches VALUES (?,?,?,?)",
            ("bad", "DWD_RV", "RED", json.dumps({"bad": True})))
        connection.commit()
    with pytest.raises(RuntimeError): db.load_active_hazards()
